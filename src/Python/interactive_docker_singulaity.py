...
def create_singularity_file():
    ...
    files = []
    while True:
        file = input("Enter the file(s) you'd like to copied to the container or leave empty to finish (%files): ")
        if not file:
            break
        files.append(file)
    ...
    post_commands = []
    while True:
        post_command = input("Enter a post-installation command or leave empty to finish: ")
        if not post_command:
            break
...
def main():
    ...
    print("Choose the container type: ")
    print("1. Docker")
    print("2. Singularity")
